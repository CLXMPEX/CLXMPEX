...
-- Escort key tiers: T1..T5 -> the item name in your inventory.
-- (confirmed from the escorts config: tierKeys mapping)
do
    local n = 0
...
-- ============================================================
--  ESCORT CONFIG  (Journey's End — Elf Mage)
-- ============================================================
local ESCORT = {
    name      = "Journeys End",                    -- internal name for escorts.create
    display   = "Journey's End",
    horsePos  = CFrame.new(4408, 6021.9, -248.5),  -- teleport here on entry, then Start
}
-- tier -> exact key item name (confirmed from tierKeys)
local KEY_TIERS = {
    T1 = "T1 Holy Key",
    T2 = "T2 Holy Key",
    T3 = "T3 Holy Key",
...
local TIER_ORDER = { "T1", "T2", "T3", "T4", "T5" }
...
local defaults = {
    ...
    autoDig         = false,
    friendOnly      = false,
    escortMode      = false,       -- run escort instead of Sunshine raid
    keyPriorities   = { "T5", "T4", "T2", "T1", "T3" },  -- P1..P5 tiers
...
-- ============================================================
--  ESCORT: read key counts + pick the next usable tier
-- ============================================================
local escortAtoms
local function getEscortAtoms()
    if escortAtoms then return escortAtoms end
    for _, d in ipairs(ReplicatedStorage:GetDescendants()) do
        if d:IsA("ModuleScript") and d.Name == "atoms"
           and d:GetFullName():find("common.store.atoms") then
            pcall(function() escortAtoms = require(d) end)
            break
        end
    end
    return escortAtoms
...
-- how many of a given key item you own
local function getKeyCount(keyItemName)
    local atoms = getEscortAtoms()
    if not atoms then return 0 end
    local atomTable = atoms.atoms or atoms
    if typeof(atomTable.datastore) ~= "function" then return 0 end
    local ok, ds = pcall(atomTable.datastore)
    if not ok or typeof(ds) ~= "table" then return 0 end
...
-- pick the first tier (by priority) that you actually have keys for
local function pickEscortKey()
    for _, tier in ipairs(State.keyPriorities) do
        local keyName = KEY_TIERS[tier]
        if keyName then
            local count = getKeyCount(keyName)
            if count > 0 then
                return tier, keyName, count
            end
        end
    end
    return nil, nil, 0
...
-- ============================================================
--  ESCORT: instant start (uses the helpers defined above)
-- ============================================================
local function startEscortInstant()
    if not R.escortCreate then
        warn("[SL] escorts.create missing — cannot start escort")
        return false
    end
    if isInRaidArea() then return true end

    local tier, keyName = pickEscortKey()
    if not keyName then
        print("[SL] No escort keys available in any priority tier — skipping")
        return false
    end

    State.startingRaid = true
    State.startingRaidAt = tick()
    print("[SL] Starting escort '" .. ESCORT.display .. "' with " .. tier .. " (" .. keyName .. ")")

    for attempt = 1, 5 do
        if isInRaidArea() then
            State.startingRaid = false
            print("[SL] In escort (instant start worked)")
            return true
        end
        -- try several arg shapes (mirrors invasion/raid create)
        pcall(function() R.escortCreate:InvokeServer(ESCORT.name, { key = keyName, friendsOnly = State.friendOnly }) end)
        pcall(function() R.escortCreate:InvokeServer(ESCORT.name, { key = keyName }) end)
        pcall(function() R.escortCreate:InvokeServer(ESCORT.name, keyName) end)
        pcall(function() R.escortCreate:InvokeServer(ESCORT.name, { tier = tier }) end)
        print("[SL] Fired escorts.create (attempt " .. attempt .. ")")
        for _ = 1, 10 do
...
local function raidCycleLoop()
    while State.running do
        ...
        if not isInRaidArea() and not State.inRaid and not State.startingRaid then
            if State.escortMode then
                -- ESCORT: fire escorts.create with the priority key
                if startEscortInstant() then
                    -- put us on the horse, then press Start
                    local _, hrp = getChar()
                    if hrp then
                        hrp.CFrame = ESCORT.horsePos
                        print("[SL] Teleported onto the horse")
                    end
                    task.wait(0.3)
                    pressInRaidStart()
                    if State.autoEquipWeapon then equipWeapon() end
                    ...
                    print("[SL] Escort started!")
                end
            else
                -- BOSS RAID: fire bossRaids.create (Sunshine Lake)
...
local function victoryLoop()
    while State.running do
        ...
        if foundVictory then
            ...
            local modeName = State.escortMode and "Journey's End (Escort)" or RAID.display
            local dropsText = State.webhookRewards and getVictoryDrops() or "Rewards hidden"
            sendWebhook(
                "Run Complete - #" .. State.runCount,
                "**" .. modeName .. "** finished!",
                5763719,
                { { name = "Drops", value = dropsText, inline = false } }
            )

            -- leave back to lobby (escort or raid), then a fresh run starts
            State.freezeUntil = tick() + 6
            if State.escortMode then leaveEscort() else leaveRaid() end
            task.wait(2)
            local tries = 0
...
-- make the GUI moveable by dragging the title bar
local dragB = Instance.new("TextButton", titleBar)
dragB.Size = UDim2.new(1, -44, 1, 0); dragB.Position = UDim2.new(0, 0, 0, 0)
dragB.BackgroundTransparency = 1; dragB.Text = ""; dragB.ZIndex = 52
...
UIS.InputChanged:Connect(function(i)
    if mDrag.on and (i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseMovement) then
        ...
        mainFrame.Position = UDim2.new(mDrag.p.X.Scale, mDrag.p.X.Offset + d.X,
                                       mDrag.p.Y.Scale, mDrag.p.Y.Offset + d.Y)
    end
end)
...
-- compact dropdown (used for the 5 key-priority slots)
local function drop(label, options, default, onSelect)
    local h = Instance.new("Frame", body)
    h.Size = UDim2.new(1, 0, 0, 44); h.BackgroundColor3 = C.card
    h.BorderSizePixel = 0; h.LayoutOrder = nextOrder(); h.ClipsDescendants = true; h.ZIndex = 51
    Instance.new("UICorner", h).CornerRadius = UDim.new(0, 8)
    local hs = Instance.new("UIStroke", h); hs.Color = C.border; hs.Transparency = 0.5
    local l = Instance.new("TextLabel", h)
    l.Size = UDim2.new(0.5, 0, 0, 44); l.Position = UDim2.new(0, 10, 0, 0)
    l.BackgroundTransparency = 1; l.Text = label; l.TextColor3 = C.textMid
    l.TextSize = 11; l.Font = Enum.Font.Gotham
    l.TextXAlignment = Enum.TextXAlignment.Left; l.ZIndex = 52
    local sf = Instance.new("Frame", h)
    sf.Size = UDim2.new(0, 70, 0, 24); sf.Position = UDim2.new(1, -78, 0, 10)
    ...
    st.Size = UDim2.new(1, -8, 1, 0); st.Position = UDim2.new(0, 6, 0, 0)
    st.BackgroundTransparency = 1; st.Text = default or options[1]; st.TextColor3 = C.sun
    ...
    for i, opt in ipairs(options) do
        ...
        ob.Size = UDim2.new(0, 70, 0, 24); ob.Position = UDim2.new(1, -78, 0, 44 + (i-1)*26)
        ob.BackgroundColor3 = C.cardHi or Color3.fromRGB(38,38,52); ob.Text = opt
        ob.TextColor3 = C.text; ob.TextSize = 11; ob.Font = Enum.Font.Gotham
        ob.BorderSizePixel = 0; ob.ZIndex = 53
    ...
    local ca = Instance.new("TextButton", sf)
    ca.Size = UDim2.new(1, 0, 1, 0); ca.BackgroundTransparency = 1; ca.Text = ""; ca.ZIndex = 54
    ca.MouseButton1Click:Connect(function()
        local open = h.Size.Y.Offset > 46
        h.Size = open and UDim2.new(1, 0, 0, 44) or UDim2.new(1, 0, 0, 44 + #options*26 + 4)
    end)
end
...
sec("Escort (Journey's End)", C.green)
tog("Escort mode", "escortMode", C.green)
